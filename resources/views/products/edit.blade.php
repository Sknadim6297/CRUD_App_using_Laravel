...
<body class="min-h-screen bg-gray-100 flex justify-center flex-col items-center">
    ...
    <div class="bg-background text-primary-foreground p-6 rounded-lg shadow-lg w-full max-w-2xl">
        ...
        <input type="text" id="post-title" name="name" value="{{$product->name}}" class="w-full px-3 py-2 rounded-lg border bg-input focus:outline-none focus:ring focus:ring-primary" placeholder="Enter product name..." required />

        <!-- Product Description -->
        <label for="post-content" class="block text-sm font-medium mt-4">Product Description</label>
        <textarea id="post-content" name="desc" class="w-full px-3 py-2 rounded-lg border bg-input focus:outline-none focus:ring focus:ring-primary" placeholder="Write your product description..." required>{{$product->description}}</textarea>
        ...
        <!-- Submit Button -->
        <button name="post" class="bg-green-500 text-white mt-4 px-4 py-2 w-full rounded-lg">Update Product</button>
    </div>
</form>
